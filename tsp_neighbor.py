...
def tsp(data):
    ...
    for i in range(size - 1):
        ...
        min_path.__delitem__(new_index)
        length += m[0]
        path.append(m[1])
        city = data.pop(new_index)
    path.append(path[0])
    length += distance(city, start)[0]
...
def description():
    return "Реалізовано метод \"найближчого сусіда\" для вирішення задачі комівояжера" \
           "В цьому методі для пошуку шляху обраховується відстань від початкової точки" \
